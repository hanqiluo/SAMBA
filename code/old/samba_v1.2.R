...
samba <- function(biomarker_dataset_template,
    ...
    #
    packages_list <- c("plyr", "dplyr", "tidyr", "data.table", "stringr", "purrr", "survey", "haven", "stringr", "readxl", "berryFunctions")
    ...
    for (i in no_biodata) {
        ...
        if (file.exists(paste0(temp_template2$directory, connector, temp_template2$data))) {
            ...
            if(temp_template2$file_format == "csv") {
                ...
                biodata <- fread(file = paste0(temp_template2$directory, connector, temp_template2$data), 
                                 header = T, sep = ",")
            }
            ...
            if(temp_template2$file_format == "sav"){
                biodata <- read_sav(paste0(temp_template2$directory, connector, temp_template2$data))
            }
            
            # read STATA
            if(temp_template2$file_format == "dta"){
                biodata <- read_dta(paste0(temp_template2$directory, connector, temp_template2$data))
            }
            
            # read excel
            if(temp_template2$file_format %in% c("xls", "xlsx")){
                biodata <- read_excel(paste0(temp_template2$directory, connector, temp_template2$data))
            }
            
            # read xpt
            if(temp_template2$file_format == "xpt"){
                biodata <- read_xpt(paste0(temp_template2$directory, connector, temp_template2$data))
            }
            ...
            output_temp <-
                cbind(
                    ...
                    svymean(~def,  mydesign, na.rm = T) %>%
                        data.frame() %>%
                        mutate( deficiency_risk_se = format(round(def * 100,  digit = 2), nsmall = 2),
                ...
                select(nutrient, subgroup, subgroup_name, n, deficiency_risk, deficiency_risk_se, mean, mean_se, geomean, geomean_lci, geomean_uci, p10, p10_se, p25, p25_se, p50, p50_se, p75, p75_se, p90, p90_se)
            ...
            if(exists("subgroup", input_data)){
              ...
              # sample sizes
                ...
                def <- 
                    ...
                    mutate(deficiency_risk =    format(round(def * 100, digits = 2), nsmall = 2), 
        ...
        inflammation_adj <- function(data, select_biomarker, template){
            ...
            if(select_biomarker %in% c("retinol_biding_protein", "serum_retinol")){
                input_data <<- 
                    ...
                    mutate(log_crp_ref = ifelse(group %in% c(3, 4), log_crp, log_crp_ref),
                           log_agp_ref = ifelse(group %in% c(3, 4), log_crp, log_agp_ref))
            }
            ...
            if(select_biomarker %in% c("serum_zinc")){
                input_data <<- 
                    ...
                    mutate(log_crp_ref = ifelse(group == 4, log_crp, log_crp_ref),
                           log_agp_ref = ifelse(group == 4, log_crp, log_agp_ref))
            }
            ...
            input_data <-
                input_data %>%
                mutate(
                    adjustment = case_when(group %in% c(1, 3, 5, 6, 7) ~ "Outside BRINDA adjustment scope",
        ...
        #final_biodata[, c("wt_temp", "def")] <- NULL
        ...
        rm(results, input_data,  operation, mydesign, envir = .GlobalEnv)
