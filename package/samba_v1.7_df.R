...
samba <- function(biomarker_dataset_template,
    ...
    for (i in no_biodata) {
        ...
        #
        # Function 2: check if each paramter is correct -----------------------
        #
        ...
        #####################
        parameters_analysis <- function(input_data, select_biomarker, template = temp_template2){
            ...
            mydesign <- survey_design(input_data = input_data, template = temp_template2)
        ...
        calculate_deficiencies <- function(input_data, reference, template) {
            ...
            for (select_biomarker in unique(reference$biomarker)) {
                ...
                if(!exists(select_biomarker, input_data)) {
                    ...
                    output_temp <- parameters_analysis(input_data = input_data, select_biomarker = select_biomarker)
                    #print(output_temp)
                    ...
                    if(((select_biomarker == "serum_ferritin") & (!is.na(template$crp) | !is.na(template$agp))) |
                       ...
                       { # inflammation adjustment
                        input_data <- inflammation_adj(input_data = input_data, select_biomarker = select_biomarker, 
                                                       template = temp_template2, operation = operation)
