...
check_variables <- 
    function(reference_table, template){
        ...
        # notification
        print(paste0("-------------Analyzing ", template$data, "-------------"))
...
transform_biodata <- function(template, input_data){
    ...
    return(list(final_biodata = as.tibble(final_biodata), template = template))
}
#
...
calculate_deficiencies <- function(input_data, 
    ...
    for (select_biomarker in unique(reference_table$biomarker)) {
        ...
        if(!exists(select_biomarker, input_data)) {
            ...
            input_data$cutoff[input_data$male == 1] <- cutoff_assign(pop = "male", reference_table = reference_table,select_biomarker = select_biomarker, input_data = input_data[input_data$male == 1, ])
            
            input_data$cutoff[input_data$np_female == 1] <- cutoff_assign(pop = "female", reference_table = reference_table,select_biomarker = select_biomarker, input_data = input_data[input_data$np_female == 1, ])
            
            input_data$cutoff[input_data$preg == 1] <- cutoff_assign(pop = "pregnant women", reference_table = reference_table,select_biomarker = select_biomarker, input_data = input_data[input_data$preg == 1, ])
            ...
            { # inflammation adjustment
                input_data <- inflammation_adj(input_data = input_data, select_biomarker = select_biomarker, template = current_biodata_template_final)
...
save_data <- function(no_biodata,
    ...
    print(paste0("-------------save `", i, "_clean_", template$file_name, "` under ", save_directory, " -------------"))
    # add results
    ...
    # To be complete
    #
    print(paste0("-------------Analysis of ", template$data, " is complete-------------"))
    print(paste0("--------------------------------------------------------------"))
