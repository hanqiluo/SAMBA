...
samba <- function(biomarker_dataset_template,
  ...
  biomarker_data <- load_templates(biomarker_dataset_template, biomarker_cutoff_template, save_directory)
  temp_template <- biomarker_data$temp_template 
  ...
  for (i in no_biodata) {
    #
    # Function 2: check if each parameter is correct -----------------------
    #
    # transpose template to an easy-to-read form
      
    temp_template2 <- check_format(temp_template, i)
    
    biodata <- import_biodata(temp_template2, connector)
    
    #######################################################################
    # check variables - Reference #########################################
    #######################################################################
    
    variables_checked <- check_variables(reference, temp_template2)
    reference <- variables_checked$reference  
    temp_template2 <- variables_checked$temp_template2 
    
    #
    # function: Create a new dataset with the variable name that I want --------------
    #
    ...
    biodata_transformed <- transform_biodata(temp_template2, biodata) 
    temp_template2 <- biodata_transformed$temp_template2
    ...
    converted_datasets <- convert_units(temp_template2, final_biodata)
    temp_template2 <- converted_datasets$temp_template2 
    ...
    #################################
    # function: adjust for inflammation  -----------------------------------d
    #################################
    ...
    data_saved <- save_data(no_biodata, final_biodata, save_directory, i, temp_template2, results_all, results, results_name)
