...
samba <- function(biomarker_dataset_template,
    ...
    #
    packages_list <- c("plyr", "dplyr", "tidyr", "data.table", "stringr", "purrr", "survey", "haven",  "readxl", "berryFunctions")
            ...
            output_temp <-
                cbind(
                    ...
                    svymean(~def,  mydesign, na.rm = T, deff = T) %>%
                        data.frame() %>%
                        mutate( deficiency_risk_df = ifelse(deff == Inf, NA, deff),
                    ...
                    svymean(~biomarker,  mydesign, na.rm = T, deff = T) %>%
                        data.frame() %>%
                        mutate( mean_df = ifelse(deff == Inf, NA, deff),
            ...
            if(exists("subgroup", input_data)){
              ...
              # sample sizes
                ...
                def <- 
                    ...
                    mutate(
                           deficiency_risk_df = ifelse(DEff.def == Inf, NA, DEff.def),
                ...
                mean <- 
                    ...
                    mutate(mean_df = ifelse(DEff.biomarker == Inf, NA, DEff.biomarker),
