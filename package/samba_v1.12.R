...
inflammation_adj <- function(input_data, select_biomarker, template){
    ...
    if(select_biomarker %in% c("serum_zinc")){
      ...
      if(nrow(zinc_psc) > 2) {
        if(!all(is.na(zinc_psc$agp))){
          spearman_agp_results <- cor.test(zinc_psc$biomarker, 
                                           zinc_psc$agp, 
        ...
        if(!all(is.na(zinc_psc$crp))){
          spearman_crp_results <- cor.test(zinc_psc$biomarker, 
                                           zinc_psc$crp, 
                                           method=c("spearman"), exact=F)
        ...
        if(!is.na(agp_correlation) && agp_correlation == 1 | 
          ...
          input_data$zinc_correction <- 1
        } else {
          input_data$zinc_correction <- 0
        }
      }
    }
    ...
    #
    # regression: when both AGP and CRP are available, other nutrients
    #
    ...
    #
    # Regression: Only AGP is available
    #
    ...
    #
    # Regression - Only CRP is available
    #
    if(!(select_biomarker %in% c("transferrin_receptor")) & is.na(template$agp) & !is.na(template$crp)){
    ...
    # apply adjustment algorithm 
      ...
      if(select_biomarker %in% c("serum_zinc")){
        input_data <- 
          input_data %>%
          mutate(adj_biomarker = 
                   case_when(group == 2 ~ ifelse(zinc_correction == 1, 
                                                 exp(log_biomarker - psc_beta2 * log_agp_diff - psc_beta3 * log_crp_diff), biomarker),
                             group == 4 ~ biomarker,
                             group != 2 & group != 4 ~ NA_real_,
                             is.na(group) ~ NA_real_)) |>
          dplyr::select(-zinc_correction)
      }
        ...
        if(!exists(select_biomarker, input_data)) {
            ...
            { # inflammation adjustment
                ...
                input_data <-
                    ...
                    dplyr::rename(biomarker = adj_biomarker) |>
